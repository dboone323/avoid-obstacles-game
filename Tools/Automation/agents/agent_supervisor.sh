...
legacy_set_agent_status() {
    ...
    while IFS= read -r key; do
        [[ -n ${key} ]] || continue
        # Use Python instead of jq for reliable JSON processing
        python3 -c "
import json
import sys
import tempfile
...
try:
    ...
    data['agents'][agent_key]['status'] = status
    data['agents'][agent_key]['last_seen'] = now
    data['last_update'] = now
    
    # Clean numeric fields
    agent_data = data['agents'][agent_key]
    for field in ['pid', 'last_seen', 'tasks_completed', 'restart_count']:
        if field in agent_data:
            try:
                # Convert strings with spaces to clean numbers
                if isinstance(agent_data[field], str):
                    agent_data[field] = float(agent_data[field].strip())
                elif isinstance(agent_data[field], (int, float)):
                    agent_data[field] = float(agent_data[field])
    ...
    sys.exit(1)
" "$key" "$status" "$now" "$STATUS_FILE" "$temp_file" && mv "$temp_file" "$STATUS_FILE"
...
record_agent_pid() {
    ...
    if ! update_agent_entry_python "${agent_script}" --set-field "pid=${pid}" >/dev/null 2>&1; then
        if command -v python3 &>/dev/null; then
            ...
            while IFS= read -r key; do
                [[ -n ${key} ]] || continue
                # Use Python for reliable PID cleaning
                python3 -c "
import json
import sys
import tempfile
...
try:
    ...
    try:
        pid = float(pid_str.strip()) if isinstance(pid_str, str) else float(pid_str)
        pid = int(pid)
    except (ValueError, TypeError):
        pid = 0
    ...
    sys.exit(1)
" "$key" "$pid" "$STATUS_FILE" "$temp_file" && mv "$temp_file" "$STATUS_FILE"
            done < <(status_keys_for "${agent_script}")
        fi
    fi
}
...
run_supervisor_loop() {
    ...
    # Update status when loop starts
    update_agent_status "agent_supervisor.sh" "active" $$ ""
    ...
    ensure_within_limits() {
        local agent_name="agent_supervisor.sh"

        # Check concurrent instances
        local running_count
        running_count=$(pgrep -f "${agent_name}" | wc -l)
        if [[ ${running_count} -gt ${MAX_CONCURRENCY} ]]; then
            echo "[$(date)] Supervisor: Too many concurrent instances (${running_count}/${MAX_CONCURRENCY}). Waiting..." >>"${LOG_FILE}"
            return 1
        fi
        ...
        if (($(echo "${load_avg} >= ${LOAD_THRESHOLD}" | bc -l 2>/dev/null || echo "0"))); then
            echo "[$(date)] Supervisor: System load too high (${load_avg} >= ${LOAD_THRESHOLD}). Waiting..." >>"${LOG_FILE}"
            return 1
        fi
    ...
    while true; do
        ...
        if ! ensure_within_limits; then
            ...
            while ! ensure_within_limits && [[ ${attempts} -lt 10 ]]; do
                echo "[$(date)] Supervisor: Waiting ${wait_time}s before retry (attempt $((attempts + 1))/10)" >>"${LOG_FILE}"
                sleep "${wait_time}"
            ...
            if ! ensure_within_limits; then
                echo "[$(date)] Supervisor: System still busy after retries. Skipping cycle." >>"${LOG_FILE}"
                sleep 60
                continue
            fi
        fi
        ...
        if [[ "${SINGLE_RUN:-false}" == "true" ]]; then
            echo "[$(date)] Supervisor: SINGLE_RUN mode - exiting after one cycle" >>"${LOG_FILE}"
            update_agent_status "agent_supervisor.sh" "stopped" $$ ""
            exit 0
        fi
        ...
        # Update supervisor status periodically
        update_agent_status "agent_supervisor.sh" "active" $$ ""
