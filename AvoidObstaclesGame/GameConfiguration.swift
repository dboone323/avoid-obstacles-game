...
struct GameConfiguration: Codable {
    ...
    struct Difficulty: Codable {
        ...
        static let `default` = Difficulty(levels: [
            DifficultyLevel(scoreThreshold: 0, spawnInterval: 1.2, obstacleSpeed: 200, scoreMultiplier: 1.0, obstacleCount: 1),
            DifficultyLevel(scoreThreshold: 10, spawnInterval: 1.0, obstacleSpeed: 250, scoreMultiplier: 1.2, obstacleCount: 2),
            DifficultyLevel(scoreThreshold: 25, spawnInterval: 0.8, obstacleSpeed: 300, scoreMultiplier: 1.5, obstacleCount: 3),
            DifficultyLevel(scoreThreshold: 50, spawnInterval: 0.6, obstacleSpeed: 350, scoreMultiplier: 2.0, obstacleCount: 4),
            DifficultyLevel(scoreThreshold: 100, spawnInterval: 0.5, obstacleSpeed: 400, scoreMultiplier: 2.5, obstacleCount: 5),
            DifficultyLevel(scoreThreshold: 200, spawnInterval: 0.4, obstacleSpeed: 450, scoreMultiplier: 3.0, obstacleCount: 6)
        ])
    }
    ...
    static func load() -> GameConfiguration {
        guard let data = UserDefaults.standard.data(forKey: configKey),
              let config = try? JSONDecoder().decode(GameConfiguration.self, from: data) else {
            return .default
        }
        return config
    }
