		...
		if [ -n "$proj" ]; then
			...
			if command -v xcpretty >/dev/null 2>&1; then
				if [ -f "$testplan_file" ]; then
					...
					xcodebuild -project "$proj" -scheme "$scheme" -destination "platform=macOS,variant=Mac Catalyst" build | xcpretty || xcodebuild -project "$proj" -scheme "$scheme" -destination "platform=macOS,variant=Mac Catalyst" build || true
				fi
			else
				if [ -f "$testplan_file" ]; then
					...
					xcodebuild -project "$proj" -scheme "$scheme" -destination "platform=macOS,variant=Mac Catalyst" build || true
				fi
			fi
		else
